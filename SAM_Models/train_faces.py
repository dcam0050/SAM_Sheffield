...
if ('.pickle' in modelPath):
    fname = '/'.join(modelPath.split('/')[:-1]) + '/' + dataPath.split('/')[
        -1] + '__' + trainName + '__' + model_type + '__exp' + str(experiment_number)
else:
    fname = modelPath + dataPath.split('/')[-1] + '__' + trainName + '__' + model_type + '__exp' + str(
...
for i in range(len(participantList)):
    ...
    (Yalli, Lalli, YtestAlli, LtestAlli) = mySAMpy.prepareData(model_type, Ntr, pose_selection,
...
for i in range(len(participantList)):
    ...
    cur.training(model_num_inducing, model_num_iterations, model_init_iterations, fname_cur, save_model, economy_save,
                 keepIfPresent=False, kernelStr=kernelString)
    mm.append(cur)
    ss = [];
    sstest = [];
    print
...
for i in range(result.shape[0]):
    ...
    for g in range(result.shape[2]):
        ...
        print('Percentage ' + participantList[i] + ' classified as ' + participantList[g] + ' = ' + str(
            confusionMatrix[i][g]))
print
